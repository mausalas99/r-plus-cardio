...
/**
 * Clear-only leftovers (inicio dates / Furosemida / indicaciones) not in the fill slots.
 * Cleared before lab-body fill so blanking '13.03.26' cannot wipe a filled labsLines date.
 */
...
function clearLabsAndMedSamples(xml, labsLines, medCells) {
  let out = xml;
  // Clear reference lab dates + leftover med-table inicio/indicación cells first.
  ...
  for (let i = 0; i < ORIG_LAB_BODIES.length; i += 1) {
    // Marker pass avoids wiping an earlier fill that matches a later ORIG body string.
...
function fillIcHojaXml(xml, payload) {
  ...
  let out = clearLabsAndMedSamples(xml, p.labsLines, p.medCells);
