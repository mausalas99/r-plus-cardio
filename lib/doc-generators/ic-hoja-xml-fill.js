...
function replaceFirstT(xml, oldVal, newVal) {
  const eOld = esc(oldVal);
  const eNew = esc(newVal);
  const plain = `<w:t>${eOld}</w:t>`;
  const preserve = `<w:t xml:space="preserve">${eOld}</w:t>`;
  const plainIdx = xml.indexOf(plain);
  const preserveIdx = xml.indexOf(preserve);
  if (plainIdx < 0 && preserveIdx < 0) return xml;
  if (preserveIdx >= 0 && (plainIdx < 0 || preserveIdx < plainIdx)) {
    return (
      xml.slice(0, preserveIdx) +
      `<w:t xml:space="preserve">${eNew}</w:t>` +
      xml.slice(preserveIdx + preserve.length)
    );
  }
  return xml.slice(0, plainIdx) + `<w:t>${eNew}</w:t>` + xml.slice(plainIdx + plain.length);
}

/**
 * Clear-only leftovers not covered by fill slots (identity-adjacent scraps).
 * Dates / Furosemida / extra indicaciones are filled via medInicio / medExtra.
 */
const ORIG_MED_CLEAR_ONLY = [];

/** Inicio sentinels in document order (replaceFirst). Trailing space is significant. */
const ORIG_MED_INICIOS = [
  '13.03.26 ', // Dapagliflozina
  '13.03.26', // Bisoprolol
  '13.03.26', // Finerrenona
  '17.03.26', // Furosemida
  '13.03.26', // Enoxaparina
  '13.03.26', // Prednisona
  '13.03.26', // Levofloxacino
  '14.03.26', // Polietilenglicol
  '16.03.26', // Ipratropio
];

const ORIG_MED_FUROSEMIDA = 'Furosemida';
const ORIG_MED_INDS_EXTRA = ['Exacerbación EPOC', 'Exacerbación EPOC', 'EPOC'];

/**
 * Unique med-table sample cells fillable from payload.medCells
 * (9 names + 9 doses + 2 indicaciones). Exact w:t match only.
 */
...
/** Prefer free-text nota; otherwise map boolean → Sí/No. */
function notaOrYesNo(nota, flag) {
  const text = nota != null ? String(nota).trim() : '';
  if (text) return text;
  return yesNo(flag);
}
...
function fillChecklistAndScores(xml, payload) {
  ...
  out = replaceT(
    out,
    ORIG.soplos,
    `Soplos: ${notaOrYesNo(c.soploNota, c.soplo)} `,
  );
  out = replaceT(
    out,
    ORIG.estertores,
    `Estertores/derrame (especificar): ${notaOrYesNo(c.estertoresNota, c.estertores)}`,
  );
  out = replaceT(
    out,
    ORIG.ascitis,
...
function clearLabsAndMedSamples(xml, labsLines, medCells, medInicioCells, medExtra) {
  let out = xml;
  // Mark med-table inicio / Furosemida / extra indicaciones before lab-date clears
  // (shared strings like "13.03.26 " would otherwise wipe Dapagliflozina's cell).
  for (let i = 0; i < ORIG_MED_INICIOS.length; i += 1) {
    out = replaceFirstT(out, ORIG_MED_INICIOS[i], `\uE000MINI${i}\uE001`);
  }
  out = replaceFirstT(out, ORIG_MED_FUROSEMIDA, '\uE000FURO\uE001');
  for (let i = 0; i < ORIG_MED_INDS_EXTRA.length; i += 1) {
    out = replaceFirstT(out, ORIG_MED_INDS_EXTRA[i], `\uE000MIND${i}\uE001`);
  }

  for (const d of ORIG_LAB_DATES) {
    out = replaceT(out, d, '');
  }
  for (const cell of ORIG_MED_CLEAR_ONLY) {
    out = replaceT(out, cell, '');
  }
  const next = Array.isArray(labsLines) ? labsLines : [];
  for (let i = 0; i < ORIG_LAB_BODIES.length; i += 1) {
    out = replaceT(out, ORIG_LAB_BODIES[i], `\uE000LAB${i}\uE001`);
  }
  for (let i = 0; i < ORIG_LAB_BODIES.length; i += 1) {
    const val = i < next.length ? String(next[i] || '') : '';
    out = replaceT(out, `\uE000LAB${i}\uE001`, val);
  }
  const nextMed = Array.isArray(medCells) ? medCells : [];
  for (let i = 0; i < ORIG_MED_CELLS.length; i += 1) {
    out = replaceT(out, ORIG_MED_CELLS[i], `\uE000MED${i}\uE001`);
  }
  for (let i = 0; i < ORIG_MED_CELLS.length; i += 1) {
    const val = i < nextMed.length ? String(nextMed[i] || '') : '';
    out = replaceT(out, `\uE000MED${i}\uE001`, val);
  }

  const inicios = Array.isArray(medInicioCells) ? medInicioCells : [];
  for (let i = 0; i < ORIG_MED_INICIOS.length; i += 1) {
    const raw = i < inicios.length ? String(inicios[i] || '').trim() : '';
    const space = /\s+$/.test(ORIG_MED_INICIOS[i]) ? ' ' : '';
    out = replaceT(out, `\uE000MINI${i}\uE001`, raw ? raw + space : '');
  }
  const extra = medExtra && typeof medExtra === 'object' ? medExtra : {};
  out = replaceT(out, '\uE000FURO\uE001', String(extra.furosemidaName || ''));
  const inds = Array.isArray(extra.indicacionesExtra) ? extra.indicacionesExtra : [];
  for (let i = 0; i < ORIG_MED_INDS_EXTRA.length; i += 1) {
    const val = i < inds.length ? String(inds[i] || '') : '';
    out = replaceT(out, `\uE000MIND${i}\uE001`, val);
  }
  return out;
}
...
function fillIcHojaXml(xml, payload) {
  ...
  let out = clearLabsAndMedSamples(
    xml,
    p.labsLines,
    p.medCells,
    p.medInicioCells,
