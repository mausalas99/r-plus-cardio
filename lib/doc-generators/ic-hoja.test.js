...
describe('fillIcHojaXml / generateIcHojaBuffer', () => {
  ...
  it('Juan Pérez export clears Ma.Elena med-table PHI (dates, indicaciones, identity)', async () => {
    const buf = await generateIcHojaBuffer({
      payload: {
        nombre: 'Juan Pérez',
        registro: '1234567-8',
        edad: '62',
        ingresoDisplay: '01/07/2026',
        fechaDisplay: '19/07/2026',
        diasInternamiento: 2,
        diasDescongestion: 1,
        inicioDescongestionDisplay: '18/07/2026',
        vexus: 0,
        congestionScore: 0,
        checklist: { soplo: false, estertores: true },
        medCells: ['Empagliflozina', '', '', '', '', '', '', '', '', '10 mg c/24h'],
        diagnosticos: ['IC descompensada'],
      },
    });
    const xml = await readDocumentXml(buf);
    assert.match(xml, /Juan Pérez/);
    assert.match(xml, /1234567-8/);
    assert.match(xml, /Soplos: No /);
    assert.match(xml, /Estertores\/derrame \(especificar\): Sí/);
    assert.doesNotMatch(xml, /0893295-0/);
    assert.doesNotMatch(xml, /Ma\. Elena/);
    assert.doesNotMatch(xml, /Anticoagulación/);
    assert.doesNotMatch(xml, /Exacerbación EPOC/);
    // Leftover med-table inicio dates (exact cells) must not remain
    assert.doesNotMatch(xml, />13\.03\.26</);
    assert.doesNotMatch(xml, />16\.03\.26</);
    assert.doesNotMatch(xml, />17\.03\.26</);
  });

  it('maps soplo/estertores boolean to Sí/No when nota is empty', () => {
    const xml = [
      `<w:t>${ORIG.soplos}</w:t>`,
      `<w:t>${ORIG.estertores}</w:t>`,
    ].join('');
    const out = fillIcHojaXml(xml, {
      checklist: { soplo: true, estertores: false },
    });
    assert.match(out, /Soplos: Sí /);
    assert.match(out, /Estertores\/derrame \(especificar\): No/);
  });
});
